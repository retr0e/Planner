...
app.post("/subjects/get-by-direction-semester", (req, res) => {
  ...
  request.query("SELECT * FROM Subject WHERE direction_id = @direction_id AND semester_id = @semester_id",
    (err, result) => {
    if(err) {
      console.error("Error executing query: ", err);
      return res.status(500).json({ok: false, reason: "Database error"});
...
app.post("/departments/update", async (req, res) => {
  ...
  request.input('department_address', mssql.VarChar, department_address);
  await request.query(
    "UPDATE Department_address SET street = @department_address WHERE department_address_id = (SELECT department_address_id FROM Department WHERE department_id = " + id + ")",
    (err, result) => {
      if (err) {
        console.error("Error executing query: ", err);
        return res.status(400).json({ ok: false, reason: err.message });
